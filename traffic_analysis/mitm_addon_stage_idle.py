...
def response(flow: http.HTTPFlow):
    ...
    if app_package == "":
        return
    analysis_type = ctx.options.analysis_type
    aaid = subprocess.run(
    ...
    latitude_longitude = regex_intro + r'57[\s.,-]689800[\s.,-]11[\s.,-]974162["\']'
    latitude = regex_intro + r'57[\s.,-]689800["\']'
    longitude = regex_intro + r'11[\s.,-]974162["\']'
        ...
        match pd_dict[personal_data]:
            ...
            case _:  # All non-regex PD
                ...
                if pd_found_in_path:
                    ...
                    separator_index = max(amp_index, comma_index, bracket_index) + 1
    ...
    if will_update_db:
        ...
        with psycopg.connect("dbname=android_tcf_traffic_analysis user=postgres password=postgres") as conn:
            with conn.cursor() as cur:
                query = """
                    INSERT INTO idle_stage (app_package, request, url, pd, analysis, date, status_code)
                    SELECT %s, %s, %s, %s, %s, %s, %s
                    ON CONFLICT (app_package,request,url,analysis,status_code) DO NOTHING;"""  # The database-table has a constraint for (app_package,request,url,analysis) being unique
